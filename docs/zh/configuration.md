...
- GitHub App credential presence，以及 GitLab project profile credential presence / write-only secret update；
...
`Runtime.MaxRunAttempts` 表示自动运行的最大总尝试次数，并且包含首次运行。例如值为 `3` 时，最多执行一次首次运行和两次自动重试。`Runtime.RetryBackoffSeconds` 与 `Runtime.MaxRetryBackoffSeconds` 控制这些尝试之间按上限截断的指数退避。
...
GitHub App private key、private key path、webhook secret，以及 GitLab project
profile 内的 token、webhook secret、webhook signing token 只接受 one-shot
replace / clear 语义：
...
旧配置中的 `Oratorio:GitHub:Token` 会被 runtime、diagnostics 和 Settings
静默忽略。它不会提供 GitHub read sync 或 write-back 能力，并会在下一次
Settings 保存 overlay 时自然消失。
...
PR/MR review run 必须由 agent 调用 `oratorio.SubmitReviewDraft`。没有发现问题时也要提交 summary-only draft：计数全为 `0`，`comments: []`，Oratorio 存储和发布的 summary 固定为 `No issues found.`。有 accepted finding 时，发布 summary 只保留 `Found N issue.` 或 `Found N issues.`，具体细节放入 inline comments。Review Draft 默认使用克制的英文工程化文案：inline comment 优先 actionable bug 和 investigation flag，避免 FYI 噪音；comment body 使用自然评审语气说明 failure mode 与影响，可用一句简短修复方向收尾，不强制分段标题。`RED` 表示较可能影响 correctness、security、data loss 或核心 workflow 的 bug；`YELLOW` 表示需要调查的 flag、maintainability risk 或置信度较低的问题；发布到 GitHub/GitLab 的 inline title 会自动加上对应红/黄灯前缀，draft 中存储的 title 保持纯文本。Inline comment 必须二选一：提供可发布为 GitHub/GitLab native suggested change 的 `suggestionReplacement`，或提供 `commentOnlyReason`（如需要人工判断、改动范围更大、无法安全 anchor 等）。`majorCount`、`minorCount`、`suggestionCount` 由服务端按 accepted findings 重算，不统计 prose-only finding 为 suggestion。Inline comment 必须锚到 diff 中可评论的 changed/context line；如果 agent 提交的 path、side、line 或 range 可被修正，tool 会以 `reviewDraftAnchorNotCommentable` 失败并返回可用 ranges，要求 agent 重新提交。Summary-only draft 不需要读取 source diff；如果 diff 暂不可用或 provider 没返回文件 patch，inline comments 会保留为 skipped warnings。若 run 完成时没有 Review Draft，Oratorio 会以 `reviewDraftRequired` 进入有上限的运行重试，且不会自动合成结论。语义或模型回合失败可复用上一条兼容 Thread；基础设施失败使用新 Thread。若非人工终止前已经持久化有效 Draft，则该 Draft 仍是权威结果，不会再次提交重复草稿。
