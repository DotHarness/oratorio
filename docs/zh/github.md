...
如果想完整使用 GitHub 集成，推荐使用 GitHub App。Token 可以用于只读导入，但 Oratorio 要写回评论、Pull Request 审阅、审阅检查或交付 Pull Request 时，需要 GitHub App。
...
**回写失败。** 确认 GitHub writes 已开启、Oratorio 使用的是 GitHub App 凭据而不只是 Token，并且 App 拥有本次操作需要的写入权限。
