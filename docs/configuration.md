...
`Runtime.MaxRunAttempts` is the maximum total number of automatic run attempts, including the initial run. For example, `3` means one initial attempt plus at most two automatic retries. `Runtime.RetryBackoffSeconds` and `Runtime.MaxRetryBackoffSeconds` define the capped exponential delay between those attempts.
...
PR/MR review runs must call `oratorio.SubmitReviewDraft`. Clean reviews still submit a summary-only draft with all counts set to `0` and `comments: []`; Oratorio stores and publishes the clean summary as exactly `No issues found.`. Reviews with accepted findings publish only `Found N issue.` or `Found N issues.` as the summary and put all actionable detail in inline comments. Review Draft prose uses restrained English by default: inline comments prioritize actionable bugs and investigation flags over FYI noise. `RED` means a likely correctness, security, data-loss, or workflow-breaking bug, while `YELLOW` means an investigation flag, maintainability risk, or lower-confidence issue; published inline comment titles are prefixed with the matching red/yellow light. Each inline comment must either provide `suggestionReplacement` that can be published as a native GitHub/GitLab suggested change, or provide `commentOnlyReason` for findings that need human judgment, a larger change, unsafe anchoring, investigation only, or a left-side/deletion anchor. The server derives `majorCount`, `minorCount`, and `suggestionCount` from accepted findings and does not count prose-only findings as suggestions. Inline comments must anchor to commentable changed/context lines in the diff; when an agent-submitted path, side, line, or range is correctable, the tool fails with `reviewDraftAnchorNotCommentable` and returns available ranges so the agent can resubmit. Summary-only drafts do not require source diff reads; when diff data or a provider file patch is unavailable, inline comments are preserved as skipped warnings. A run that completes without any Review Draft fails with `reviewDraftRequired` and uses the bounded run retry policy; Oratorio never synthesizes the conclusion. Semantic/model retries may resume the preceding compatible thread, while infrastructure failures use a fresh thread. A valid draft persisted before a non-operator terminal failure remains authoritative and prevents duplicate draft submission.
